...
def client_handler(gui: Gui, state_id_list: list) -> None:
    """
    Runs in a separate thread and periodically calls to update the conversation.
...
def update_conv(state: State) -> None:
    """
    Update the conversation table with the latest content from the main program.
...
<|{conversation}|table|show_all|width=100%|rebuild|selected={selected_row}|>
|>
|>
"""
...
# Periodically update the conversation on a separate thread
t = Thread(
